...
def get_unified_data(
    ...
    def create_unified_dataset(out_path: str, info_path: str, info_dict: dict, path_col: str):
        ...
        train_split_df = pd.read_csv(
            os.path.join(in_path, "meta", "train_indexes.csv"), index_col="indexes"
        )
        train_split_df["split"] = "train"
        val_split_df = pd.read_csv(
            os.path.join(in_path, "meta", "valid_indexes.csv"), index_col="indexes"
        )
        val_split_df["split"] = "val"
        test_split_df = pd.read_csv(
            os.path.join(in_path, "meta", "test_indexes.csv"), index_col="indexes"
        )
        ...
        # Grouped labels
        meta_df["diagnosis_simplified"] = meta_df["diagnosis"].apply(
            lambda d: "melanoma" if "melanoma" in d else d
        )
        diagnosis_groups = {
        ...
        meta_df["vascular_structures_grouped"] = meta_df["vascular_structures"].map(
            vascular_structures_groups
        )
        pigmentation_groups = {
            "absent": "absent",
        ...
        meta_df["regression_structures_grouped"] = meta_df["regression_structures"].map(
            regression_structures_groups
        )
        ...
        tasks_mapper = {
            task["task_name"]: {lab: idx for idx, lab in task["labels"].items()}
        ...
        # Paths
        meta_df["original_filepath"] = meta_df[path_col].apply(
            lambda p: os.path.join(in_path, "images", p)
        )
        meta_df.drop(columns=["clinic", "derm"], inplace=True)
        meta_df.set_index("original_filepath", inplace=True)

        # Add annotations
        add_annot_cols = set(meta_df.columns) - set(
            ["split", *[task["task_name"] for task in info_dict["tasks"]]]
        )
        ...
        def get_writer_input(path: str):
            ...
            add_annot = {
                ...
                **{col: df_row[col] for col in add_annot_cols},
            }
        ...
        with UnifiedDatasetWriter(out_path, info_path) as writer:
            ...
            for paths in tqdm(
                np.array_split(all_paths, len(all_paths) // batch_size),
                desc=f"Processing {info_dict['name']}",
            ):
    ...
    create_unified_dataset(
        out_path=out_paths[0], info_path=info_paths[0], info_dict=info_dicts[0], path_col="clinic"
    )
    create_unified_dataset(
        out_path=out_paths[1], info_path=info_paths[1], info_dict=info_dicts[1], path_col="derm"
    )
